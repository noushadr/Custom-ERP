...
A separate section of five modules (originally six — see module 5 below), gated stricter than every other admin-tier feature: Employees and Team Leads must not view, access, or modify any of them. Enforced via a dedicated permission per module (e.g. `clients.manage`) that is granted to no role's explicit array except by deliberate exception — Super Admin always gets every module implicitly via its "every known permission" grant, and HR/Manager is additionally, explicitly granted **Clients & Projects (+ Client Health) and Payroll** — never a hardcoded role check, so a custom role could still be granted any of these permissions later without code changes. **Agency Reporting and Finances stay Super-Admin-only** — HR/Manager must not even see those two nav entries; frontend nav visibility enforces this specifically for those two via `_superAdminOnlyLabels` in `main.dart` (updated 2026-08-22 when Clients & Projects/Payroll were removed from that set — see their bullets below), while Clients & Projects/Payroll rely purely on the permission grant like every other shared admin-tier feature.
