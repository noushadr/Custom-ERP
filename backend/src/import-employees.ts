...
// (Employee (hr.employee).xlsx). See scratchpad xlsx-inspect/inspect.js for
// how summary.json and images/ were produced.
const SOURCE_DIR =
...
// People who manage at least one other person in this roster get "Team Lead".
// This is derived from the "Manager" column, not asserted.
function buildRoleAssignments(rows: SourceRow[]): Map<string, string> {
  const managerNames = new Set(
    rows.map((r) => r.manager).filter((m): m is string => !!m),
  );
  const roles = new Map<string, string>();
  for (const row of rows) {
    if (row.name === 'Noushad Ranani') {
      roles.set(row.name, 'Super Admin');
    } else if (/hr manager/i.test(row.jobTitle)) {
      roles.set(row.name, 'HR/Manager');
    } else if (managerNames.has(row.name)) {
      roles.set(row.name, 'Team Lead');
    } else {
      roles.set(row.name, 'Employee');
    }
  }
  return roles;
}

function buildEmailAssignments(rows: SourceRow[]): Map<string, string> {
...
async function run() {
  ...
  // --- Clean up the earlier test fixtures (Jane Doe + placeholder org data) ---
  const testUser = await userRepo.findOne({
    where: { email: 'jane.doe@zeracreative.com' },
  });
  if (testUser) {
    await employeeRepo.delete({ userId: testUser.id });
    await userRepo.delete({ id: testUser.id });
    console.log('Removed test fixture: jane.doe@zeracreative.com');
  }
  for (const name of ['Platform', 'Web']) {
    const team = await teamRepo.findOne({ where: { name } });
    if (team) await teamRepo.delete({ id: team.id });
  }
  for (const name of ['Engineering', 'Human Resources']) {
    const dept = await departmentRepo.findOne({ where: { name } });
    if (dept) await departmentRepo.delete({ id: dept.id });
  }
  ...
  const roleAssignments = buildRoleAssignments(rows);
  const emailAssignments = buildEmailAssignments(rows);
