...
describe('ClientsService', () => {
  ...
  describe('getProjectsNeedingRenewalReminder', () => {
    ...
    it('matches an active retainer renewing within the window', async () => {
      projectRepository.findAll.mockResolvedValue([
        buildProject({
          type: ProjectType.RETAINER,
          status: ProjectStatus.ACTIVE,
          renewalDate: isoDaysFromNow(5),
        }),
      ]);

      const result = await service.getProjectsNeedingRenewalReminder(7);

      expect(result).toHaveLength(1);
      expect(result[0].clientName).toBe('Acme Inc');
    });

    it('excludes a one-time project even with a renewalDate set', async () => {
      projectRepository.findAll.mockResolvedValue([
        buildProject({
          type: ProjectType.ONE_TIME,
          status: ProjectStatus.ACTIVE,
    ...
    it('excludes a retainer already reminded for this exact renewalDate', async () => {
      const renewalDate = isoDaysFromNow(5);
      projectRepository.findAll.mockResolvedValue([
        buildProject({
          type: ProjectType.RETAINER,
          status: ProjectStatus.ACTIVE,
          renewalDate,
          lastRenewalReminderSentFor: renewalDate,
        }),
      ]);
    ...
    it('re-includes a retainer once renewalDate changes after a prior reminder', async () => {
      projectRepository.findAll.mockResolvedValue([
        buildProject({
          type: ProjectType.RETAINER,
          status: ProjectStatus.ACTIVE,
          renewalDate: isoDaysFromNow(5),
          lastRenewalReminderSentFor: isoDaysFromNow(-30),
        }),
      ]);
    ...
    it('excludes a renewal date outside the daysBefore window', async () => {
      projectRepository.findAll.mockResolvedValue([
        buildProject({
          type: ProjectType.RETAINER,
          status: ProjectStatus.ACTIVE,
          renewalDate: isoDaysFromNow(30),
        }),
      ]);
  ...
  describe('markRenewalReminderSent', () => {
    it('stamps lastRenewalReminderSentFor with the project\'s current renewalDate', async () => {
      projectRepository.findById.mockResolvedValue(
        buildProject({ id: 'p1', renewalDate: '2026-09-01' }),
      );

      await service.markRenewalReminderSent('p1');

      expect(projectRepository.save).toHaveBeenCalledWith(
        expect.objectContaining({ lastRenewalReminderSentFor: '2026-09-01' }),
      );
    });
  });
});
